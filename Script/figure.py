...
def plt_figure9(ax, recorded_max_mean1=0, recorded_max_mean2=0, recorded_max_mean3=0, recorded_max_mean4=0,
    ...
    """在指定坐标轴上绘制7条曲线和放大子图"""
    # 设置主图属性
    ax.set_xlabel('Experiment batch', fontsize=20)  # 适当缩小字体以适应子图
    means = []
    ...
    ax.tick_params(axis='both', labelsize=20)  # 缩小刻度字体
    ax.set_ylim([0, 100])
    
    # 定义颜色和数据集
    #colors = ['blue', 'green', 'olivedrab', 'darkgoldenrod', 'orange', 'coral', 'red']
    #colors = ['blue', 'green', 'olivedrab', 'darkgoldenrod', 'red']
...
def plot_figure_with_results(data,c1='lightblue',c2='blue',title='',ylabel = '',exp_num=1,target=80,plot_value = False,x1=0.20,y1=0.10,x2=0.275,y2=0.19):
    mean_line = np.mean(data, axis=0)
    plt.title(title,fontsize=20)
    plt.xticks (fontsize=20)#, fontname='Times New Roman'
    plt.yticks (fontsize=20)
